...
interpolation, against policy 876's own real (age, term) grid - before this design
...
Usage: python3 query/validate_premium_lookup.py
"""
...
def main():
    lookup = json.loads(LOOKUP_PATH.read_text())["876"]
    ...
    print(f"{len(errors)} points testable (had a usable bracket with the rest of the grid), "
