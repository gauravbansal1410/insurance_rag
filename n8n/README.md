...
A second, independent entry point into the same `/chat` endpoint, for the standalone static frontend (`index.html`, repo root) instead of n8n's own chat widget — see `docs/query_architecture.md`'s "Runtime orchestration" section for why this is a separate workflow rather than reusing `insurance_rag_chat.json`. Three nodes: **Webhook** (plain `n8n-nodes-base.webhook`, `POST` at path `insurance-rag-frontend`, `options.allowedOrigins: "*"` for CORS since GitHub Pages is a different origin from n8n's own domain) → **HTTP Request** (`POST /chat` on the Python service, forwarding the webhook body's `session_id`/`message` straight through — no `sessionId`/`chatInput` translation, since this workflow's caller isn't n8n's chat-widget convention) → **Respond to Webhook** (`respondWith: allIncomingItems`, returns the service's raw JSON response — `{done, reply, top3?}` — rather than reshaping it, since the static page does its own rendering).

**Import:** n8n UI → Workflows → Import from File → select this JSON, same as `insurance_rag_chat.json`. **Not yet verified against the live n8n UI** — same caveat as above, plus:
- `allowedOrigins`/`respondWith` field names and the plain Webhook node's exact options schema on this n8n version (2.23.3) haven't been confirmed — if CORS errors show up in the browser console when calling from GitHub Pages, check this first.
- The production webhook URL (`https://gaurav-n8n.duckdns.org/webhook/insurance-rag-frontend`, assumed from n8n's usual `/webhook/<path>` convention) needs confirming against the actual URL n8n shows after activating the workflow, and `index.html`'s `WEBHOOK_URL` constant updated to match if it differs.
- The workflow needs to be **active** (not just imported) for the production webhook URL to respond — n8n's default `/webhook-test/<path>` URL only fires once per manual "Execute workflow" click, same test-webhook caveat noted in `docs/infra-baseline.md`'s n8n mechanics section.
