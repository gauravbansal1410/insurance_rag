...
- To run stages individually (e.g. re-precomputing rerank scores after a schema change, without redoing extraction):
  ...
  - Precomputed rerank scores: `chunking/precompute_rerank_scores.py <chunks.json> [chunks.json ...]` — reranks chunks against the 8 fixed `concern_tags` (`query/concern_tags.py`), merges into `chunking/precomputed_rerank_scores.json`, checkpointing to disk after every tag (not just once at the end — confirmed necessary 2026-07-31, see `docs/progress/20260731-progress.md`) and skipping any tag already fully covered on a restart. Exists so the live query path never makes a per-query Voyage rerank call — see `docs/query_architecture.md`'s "Reranking data source" section. Same free-tier batching/pacing as embedding — expect **hours**, not minutes, for a full category at this corpus's chunk counts. **Run this on the Oracle VM, not your laptop** — it's a long unattended job, and a laptop that sleeps mid-run kills it (confirmed the hard way). See `docs/infra-baseline.md`'s "insurance_rag specifically" section for how to launch it there detached from your SSH session (`setsid` + `loginctl enable-linger`) so it survives disconnecting.
...
- Rebuild the precomputed premium lookup (regular premium + level sum assured only, per policy — see `docs/query_architecture.md` step 5): `python3 query/build_premium_lookup.py` — builds one `query/premium_lookup.json` entry per policy listed in the script's own `POLICIES` config (876/859/875/878/954 as of 2026-08-11, each from its own full scrape), each merging that policy's `extracted/layer1_<id>.json` brochure sample-premium rows with its own `docs/progress/ground-truth/<id>_scraped_premiums.csv`. Policies whose age x term grid is instead *derived* from an already-built sibling's grid (valid only once shape-transfer between the two is confirmed — see `docs/query_architecture.md`'s open questions) go in the separate `DERIVED_POLICIES` list (877 derived from 878, 955 derived from 954, both as of 2026-08-11). To add a new policy, add a `POLICIES` (or `DERIVED_POLICIES`) entry (reference SA, ground-truth CSV path, rebate-table navigation config) rather than editing the build logic. Re-run whenever a ground-truth CSV gains rows (no Gemini/Voyage calls, purely local). `python3 query/validate_premium_lookup.py [policy_id]` (defaults to 876) leave-one-out validates one policy's rebuilt age x term grid against itself (checks the interpolation method's accuracy, not just that the file parses) — run it per policy after any rebuild before trusting the new numbers. For a derived policy, leave-one-out isn't meaningful (most points are derived, not real) — validate instead with a handful of real out-of-sample scrapes checked against the derived grid's prediction (see `docs/schema.md`'s premium-lookup artifact section for how 877/955 were validated this way).
- Run the query service locally for testing (wraps `query/`'s steps 3-8 behind HTTP — see `docs/query_architecture.md`'s "Runtime orchestration" section): `pip install fastapi uvicorn --break-system-packages`, then from `service/`: `set -a && source ../.env && set +a && uvicorn main:app --host 127.0.0.1 --port 8000` (needs `qdrant-tunnel` running first, same as any script touching Qdrant). `GET /health` for a smoke check; `POST /query` for a direct one-shot call with a full profile; `POST /chat` for turn-based slot-filling (`{"session_id": ..., "message": ...}`, `message: null` to start).
- **Deployed on the Oracle VM** (2026-08-01) as a systemd *user* service — see `docs/infra-baseline.md`'s "insurance_rag specifically" section for the full setup (bound to the Docker bridge IP, not `0.0.0.0`/`127.0.0.1`; the host firewall rule needed for n8n's container to reach it). Restart after deploying new code: `ssh ... "cd ~/insurance_rag && git pull && systemctl --user restart insurance-rag-service.service"`. `n8n/insurance_rag_chat.json` is the imported, working n8n workflow that calls it — see `n8n/README.md`.
...
├── service/               # FastAPI wrapper around query/ - /query and /chat endpoints, run_query_pipeline.py, chat_session.py - deployed on the Oracle VM as a systemd service
├── n8n/                   # n8n workflow JSON (Chat Trigger -> HTTP Request -> Set) that calls service/, plus deployment notes
├── CLAUDE.md              # This file
