...
- Rebuild the precomputed premium lookup (v1 pilot, policy 876, regular premium + level sum assured only — see `docs/query_architecture.md` step 5): `python3 query/build_premium_lookup.py` — merges `extracted/layer1_876.json`'s own brochure sample-premium rows with `docs/progress/ground-truth/876_scraped_premiums.csv` into `query/premium_lookup.json`. Re-run whenever the ground-truth CSV gains rows (no Gemini/Voyage calls, purely local). `python3 query/validate_premium_lookup.py` leave-one-out validates the rebuilt table against itself (checks the interpolation method's accuracy, not just that the file parses) — run it after any rebuild before trusting the new numbers.
