...
- *Layer 2 — normalized decision layer.* Identical schema across all categories, computed at ingestion time from Layer 1 via a **separate** Gemini call (validated 2026-07-12 — see ingestion step 2 below; original plan assumed one combined call, testing showed a second, independent call from Layer 1's JSON alone works better and is simpler). Some fields are direct copies of Layer 1 bounds (deterministic filter facts), some are restructured Layer 1 language (payout mechanics), and some are genuinely interpretive judgments (concern tags). This is the layer the query pipeline's deterministic filter and sort logic actually run against — Layer 1 is the source of truth, Layer 2 is what's queried. Full schema in `docs/schema.md`.
...
2. Extraction: Gemini `flash-lite-latest`, PDF-native input (upload the source PDFs directly via the Files API, not `pdftotext`-flattened text). Structured JSON against a core schema (premium, term, sum assured, eligibility) plus category-specific extensions (term / money-back / whole-life / endowment each have different benefit structures — decide extension fields before writing the extraction prompt). Full Layer 1/2 schema, document merge rule, and extraction-rule caveats: see `docs/schema.md`.
   - **Model choice validated 2026-07-12** against term-assurance extraction: `flash-lite-latest` with a well-trapped prompt (13 extraction-trap rules, see `docs/schema.md`) achieved fully consistent, repeatable output across multiple runs on 2 test policies. `gemini-3.1-pro` was not comparable — it has zero free-tier quota on this project's API key, so extraction stays on flash-lite for the $0 cost target rather than switching to Pro's paid tier.
   - **PDF input beats extracted text for this task.** Same prompt, same model: PDF-native input (`client.files.upload` + attach to the generate_content call) reliably found every distinct premium table (7/7 clean runs across both test policies), while `pdftotext -layout` text input missed one table roughly half the time (3/6 runs) on a document where two tables sit back-to-back under one shared heading with no visual break in the flattened text. Use PDF input for ingestion; treat text-extraction as a fallback only if PDF upload isn't viable for some future document type.
2b. Layer 2 derivation: a second, separate Gemini `flash-lite-latest` call, given ONLY the Layer 1 JSON as input — no source PDFs, no `pdftotext`. See `docs/prompts/prompt_b.txt`.
   - **Source docs deliberately withheld, validated 2026-07-12.** An earlier version attached policy_doc + brochure PDFs to this call too, on the assumption Group A's concern-tag reasoning needed brochure "positioning" language Layer 1's schema doesn't capture. Testing found this added no measurable benefit (identical Group A output to the JSON-only version, 4/4 runs) while introducing a real bug: the model would sometimes re-derive Group C bounds from the raw PDFs instead of copying Layer 1's already-resolved value, reintroducing a general-rule-vs-exception ambiguity Layer 1 had already fixed (e.g. picking a conditional Sum Assured minimum instead of the general one, meant to be a byte-for-byte copy). For term assurance, Layer 1's JSON alone carries everything Layer 2 needs — `structural_variant`, `maturity_benefit`, `surrender_value_applicable`, etc. already discriminate the small concern-tag space that applies. Revisit this if a future plan category's Group A tagging turns out to need brochure-only language Layer 1 doesn't capture (see `docs/prompts/appendix/prompt_b_pdf_deprecated.txt`, kept for that scenario).
...
- ~~Current Gemini 3.1 Pro pricing and rate limits.~~ Resolved 2026-07-07: Gemini 3.1 Pro lost free-tier API access April 1, 2026. Now $2/$12 per 1M tokens up to 200K context, $4/$18 above.
- ~~Whether to extract with Gemini Pro (paid) or Flash-Lite (free).~~ Resolved 2026-07-12: Flash-Lite, confirmed via testing on 2 term-assurance policies — see the ingestion step 2 note above. Pro comparison wasn't possible (zero free-tier quota on this key) and wasn't needed once Flash-Lite + PDF input + a trapped prompt proved reliable.
