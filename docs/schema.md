...
**877 built the same day WITHOUT its own full scrape** — `query/build_premium_lookup.py`'s new `DERIVED_POLICIES` list rescales an already-built sibling's `age_term_grid` by a factor calibrated against a small set of real anchor points, instead of building from a full scrape of its own. Valid only because shape-transfer was confirmed first (877-vs-878, both `credit_life`: 0.1-2.7% deviation at every one of 877's 6 real spot-check points — see `docs/query_architecture.md`'s open questions). 877's `age_term_grid` = 878's grid x 1.262 (the calibrated scale factor; the 6 real anchor ratios ranged 1.2416-1.2766, matching the earlier finding directly), with the 6 real anchor points substituted back in at their own (age, term) — `age_term_grid` entries carry `"source": "derived_from_878"` for every point that isn't a real anchor, so this is auditable, not silently blended in as if it were scraped data. `sa_scaling` is still built normally from 877's own real, independently-extracted rebate table — only the age x term axis is derived, since that's the only axis shape-transfer actually applies to (the SA-scaling formula was already known to be policy-specific, never assumed to transfer). `built_from.scale_factor`/`anchor_ratio_min`/`anchor_ratio_max` are logged into the built artifact itself, not just this doc. **Out-of-sample validated 2026-08-11** — 4 points deliberately chosen outside the 6 calibration anchors (two at the grid's floor term, one mid-age at the upper term, one at the domain's outer corner) all landed within 0.31-1.88% of the derived grid's prediction, confirming the derivation is accurate away from its anchors, not just internally consistent at them (the earlier gap flagged when 877 was first built).

**954 built 2026-08-11** from a real 48-point scrape covering its full valid (age, term) domain (entry_age 18-65, policy_term 10-40, capped by `age + term <= age_at_maturity_max`=80) — no Layer 1 brochure points merged in, since Layer 1's own `sample_illustrative_premiums` for 954 is all at a 1Cr baseline, not 954's own `sum_assured_min` (50L) reference. Leave-one-out: median error 1.47%, mean 3.19%, max 20.13% at age=35/term=10 — the same domain-floor edge-sensitivity pattern every policy's grid has shown at its own thinnest term. One real gap: **age=40 (DOB 15/01/1986) couldn't be scraped** — the live calculator's term dropdown stayed unselectable for Level SA at that specific DOB, reproduced across incognito, cleared cookies, and a different browser, so confirmed server-side/DOB-specific rather than a client-side session bug. Left unscraped rather than forced; the build's own age-axis interpolation fills it from the real age=35/age=45 rows on either side. Building 954 also surfaced a genuinely new rebate-table shape: **`high_sum_assured_rebate_table.dimensions` can be just `["sum_assured_type", "age_band"]`, with no `premium_payment_option_group` level at all** (single/regular/limited all share one rebate %) — unlike 876/875/877/878's 3-dimension shape. `build_sa_bands_for_type_a()` in `build_premium_lookup.py` now branches on the table's own `dimensions` field rather than assuming a fixed nesting depth.

**955 built the same day, derived from 954** — same `DERIVED_POLICIES` mechanism as 877/878 (955-vs-954 was the tightest same-family shape-transfer match found so far, 0.7-2.0% at every previously-checked spot-check point). Scale factor 0.8555, the 7 real anchor ratios ranged 0.8481-0.8649. Out-of-sample validated the same day with 4 points outside the anchors (two at floor term, one mid-age at the upper term, one at the domain's outer corner age=65/term=15, exactly at `age_at_maturity_max`=80): 0.64-3.46% error, confirming the derivation the same way 877's was confirmed.

**Real bug found and fixed 2026-08-13: `premium_lookup.json` was keyed wrong for 954/955.** `build_premium_lookup.py` used to key each entry by its `POLICIES`/`DERIVED_POLICIES` config's own short label (`"954"`, `"955"`), but `query/premium_interpolation.py`'s `interpolate_premium()` looks an entry up by `layer1_record["policy_id"]` — the policy's real extracted `policy_id`, which for these two is a LIC UIN (`512N351V02`, `512N350V02`), not `"954"`/`"955"` (see the "Registration Number" extraction-rule caveat below for why). The dictionary lookup silently missed every time, for every query against either policy, falling all the way through to the original sparse two-point linear interpolation instead of the real 48-point/derived grid — with no error, no log line, nothing to catch it short of noticing the numbers looked off. **Fixed at the source**: `build_premium_lookup.py` now reads each policy's own canonical `policy_id` from its `layer1_path` JSON and keys the output artifact by that, not the config's short label (`DERIVED_POLICIES`' `derive_age_term_from` — written using the short label for readability — resolves through a small id map built during the same pass). Confirmed fixed: `interpolate_premium()` now correctly returns a real bilinear estimate for 954 at (age 31, term 30, regular) where it previously excluded the candidate outright.

**A second, related bug in the same fix: `available_terms()`/`available_payment_options_for_term()` (`query/premium_interpolation.py`) only ever checked Layer 1's sparse `sample_illustrative_premiums` table, never `premium_lookup.json`'s much richer real grids** — even for policies whose key matched correctly (876/875, both with real regular-premium data out to term=40). These two functions back the chat's live term/payment-option validation (`service/chat_session.py`), so a real, servable combination (e.g. term=30/regular on a 31-year-old, ₹1Cr cover) was being rejected as unsupported, forcing users down a narrower path with fewer eligible policies — the two bugs compounded: even once 954's key was fixed, the availability check still wouldn't have told a user term=30/regular was an option. Fixed by rewriting both functions to route every candidate (term, payment_option) pair through `interpolate_premium()` itself — the real step-5 dispatch, including the precomputed lookup — rather than re-scanning the sample table in a second, independent path that could (and did) drift out of sync. One source of truth for "is this available" now, not two.

## Ground-truth validation datasets (not extracted content — deliberately outside Layer 1/2/3)
`docs/progress/ground-truth/` holds manually-scraped real data used to validate extraction/interpolation accuracy — never loaded into Qdrant, never treated as an extracted field. `876_scraped_premiums.csv`: real premium quotes pulled from LIC's live online quote calculator for policy 876 (Digi Term), schema designed to scale across policies (`category`, `gender`, `smoker_status`, `sum_assured_type`, split `ppt`/`policy_term`, `scraped_date`/`source` for provenance) — see `docs/query_architecture.md`'s open questions for what it's been used to confirm so far, and the premium-lookup artifact above for what it feeds. `859_scraped_premiums.csv` (added 2026-08-08): same schema, 52 real quotes covering its full valid (age, term) domain at the reference SA (scraped in one pass rather than 876's scatter-then-gap-fill approach, since 859's domain is a clean triangle) plus 4 real cross-SA quotes at higher sum assureds (10L/15L/25L, added same day) to QA-check the SA-scaling formula the same way 876's ground truth does — see the premium-lookup artifact above. `875_scraped_premiums.csv` (added 2026-08-08): 33 real quotes, its full valid (age, term) domain, all at its `sum_assured_min` reference — used for its own lookup and for the shape-transfer comparison against 876 (see `docs/query_architecture.md`'s open questions). `877_scraped_premiums.csv` (6-point spot-check, added 2026-08-10, not extended further — see the premium-lookup artifact above for why): `structural_variant: credit_life`, no Regular Premium option at all (confirmed against its own Coverage page), scraped under Limited Premium/PPT=10/8% loan interest instead, matching 877's own brochure sample-illustrative-premium convention. `878_scraped_premiums.csv` (started as the same 6-point spot-check 2026-08-10, extended to a full 24-point grid 2026-08-11, restricted to terms 15-30 so PPT=10 stays valid throughout): same shape/scope as 877's, `structural_variant: credit_life`, no Regular option. The 6 overlapping points between the spot-check and the full scrape matched exactly, confirming scrape reproducibility. `954_scraped_premiums.csv` (started as a 7-point spot-check 2026-08-10, extended to a real 48-point full scrape 2026-08-11): regular premium, its own `sum_assured_min` reference (50L — no free brochure cross-check available at this SA, since 954's own brochure sample table uses a 1Cr baseline instead). Covers every age band 18-65 across its full valid term range except age=40, which the live calculator wouldn't allow (see the premium-lookup artifact above for the DOB-specific dropdown issue). Every overlapping point between the original spot-check and the full scrape matched exactly, same reproducibility confirmation as 878's. `955_scraped_premiums.csv` (7-point spot-check, added 2026-08-10, not extended further — 955's own grid is derived from 954's instead, see the premium-lookup artifact above): regular premium, its own `sum_assured_min` reference (25L) — one flagged discrepancy: the live calculator's UI labeled this quote "Online", but 955's own brochure states offline-only purchase through licensed agents; recorded `distribution_channel: offline`, trusting the brochure over the calculator UI's label (the calculator's own labeling isn't guaranteed to reflect a real distribution-channel price differential the way the printed rebate table does). 877/955's spot-checks now each feed a *derived* lookup (see the premium-lookup artifact above) rather than needing their own full grid; both derivations are out-of-sample validated as of 2026-08-11, not just calibrated at their anchors.
